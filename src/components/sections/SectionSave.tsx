...
const SectionSave = () => {
    ...
    <MenuEl>
        <Button onClick={() => {
            ipcRenderer.sendSync('SAVE_FILE', JSON.stringify({...animationData, variojsVersion:pjson.version}, null, 1))
        }}>Save to file</Button>
       <Button onClick={() => {
                copyToClipBoard(JSON.stringify({...animationData, variojsVersion:pjson.version}, null, 2));
