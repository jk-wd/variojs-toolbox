...
import socketConnect from "../../../../src/socket-connect";
...
const isVersionValid = (version) => {
  return (
    major(version) === major(pjson.version) &&
    minor(version) === minor(pjson.version)
  );
};
  ...
  init: async (initCallback, scrollCallback, port) => {
    ...
    ws.addEventListener("message", function (event) {
      ...
      if (dataParsed.action === "siteData") {
        receivedData = true;
        if (
          !dataParsed.payload.animationData.variojsVersion ||
          !isVersionValid(dataParsed.payload.animationData.variojsVersion)
        ) {
